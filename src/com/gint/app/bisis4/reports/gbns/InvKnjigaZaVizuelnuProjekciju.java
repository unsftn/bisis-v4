...
public class InvKnjigaZaVizuelnuProjekciju extends Report {
  ...
  public void handleRecord(Record rec) {
	    ...
	    vrDok = vrDok+", "+rec.getSubfieldContent("115g"); 
	    if (vrDok == null)
	    	vrDok = " ";
	    ...
	    for (Primerak p : rec.getPrimerci()) {
	       ...
	       if (p.getInvBroj().substring(0, 2).compareToIgnoreCase("31")==0){
	    		  if(p.getInvBroj().substring(5, 7).compareToIgnoreCase("05")!=0)
	    			  continue;
	      }else if (p.getInvBroj().substring(2, 4).compareToIgnoreCase("05")!=0){
	    	  continue;
	      }
...
//	    ******************    NABAVKA NIJE ZAVRSENA   *************************
  ...
  private String name;
  private static Log log = LogFactory.getLog(InvKnjigaZaVizuelnuProjekciju.class);
@Override
