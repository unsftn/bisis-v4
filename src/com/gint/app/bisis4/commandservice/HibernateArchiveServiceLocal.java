...
public class HibernateArchiveServiceLocal implements Service{
	
	private static SessionFactory sessionFactory = null;
	private static Log log = LogFactory.getLog(HibernateArchiveServiceLocal.class.getName());
    ...
    try {
      ...
      Configuration cfg = new Configuration().configure(HibernateServiceLocal.class.getResource(
        "/com/gint/app/bisis4/client/circ/model/hibernate.cfg.xml"));
      	cfg.setProperty("hibernate.connection.driver_class", iniFile.getString("database-archive", "driver"));
        cfg.setProperty("hibernate.connection.url", iniFile.getString("database-archive", "url"));
        cfg.setProperty("hibernate.connection.username", iniFile.getString("database-archive", "username"));
        cfg.setProperty("hibernate.connection.password", iniFile.getString("database-archive", "password"));
