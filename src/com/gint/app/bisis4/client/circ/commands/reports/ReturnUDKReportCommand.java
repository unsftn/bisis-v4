...
public class ReturnUDKReportCommand extends HibernateCommand {
   ...
   */	
	...
	public ReturnUDKReportCommand(Date start, Date end, Object location){
		this.start = start;
		this.end = end;
		...
		try{
		 ...
		 if (!location.equals(" ")) {
				...
				String query1 = "select l.ctlgNo from Lending l "
						+ " where (l.returnDate between :start and :end)and " 
						+ " (l.location.name= :loc)";
				...
				String query2 =  "select l.ctlgNo from Lending l "
					+ " where (l.returnDate between :start and :end)";
		  ...
		  tx.commit();
		  log.info("ReturnUDK report");
		}catch (Exception e){
			log.error("ReturnUDK report failed");
			log.error(e);
		}
	}
